...
bool distanceDetailedCallback(fcl::CollisionObject *o1,
  ...
  const CollisionGeometryData *cd1 = static_cast<const CollisionGeometryData *>(
      o1->collisionGeometry()->getUserData());
  const CollisionGeometryData *cd2 = static_cast<const CollisionGeometryData *>(
      o2->collisionGeometry()->getUserData());
  ...
  if (cdata->req->active_components_only) {
    const robot_model::LinkModel *l1 =
        cd1->type == BodyTypes::ROBOT_LINK
            ? cd1->ptr.link
            : (cd1->type == BodyTypes::ROBOT_ATTACHED
                   ? cd1->ptr.ab->getAttachedLink()
                   : NULL);
    const robot_model::LinkModel *l2 =
        cd2->type == BodyTypes::ROBOT_LINK
            ? cd2->ptr.link
            : (cd2->type == BodyTypes::ROBOT_ATTACHED
                   ? cd2->ptr.ab->getAttachedLink()
                   : NULL);

    // If neither of the involved components is active
    if ((!l1 ||
         cdata->req->active_components_only->find(l1) ==
             cdata->req->active_components_only->end()) &&
        (!l2 ||
         cdata->req->active_components_only->find(l2) ==
             cdata->req->active_components_only->end())) {
      return false;
    } else {
      if (!l1 ||
  ...
  if (cdata->req->acm) {
    ...
    bool found =
        cdata->req->acm->getAllowedCollision(cd1->getID(), cd2->getID(), type);
    if (found) {
      // if we have an entry in the collision matrix, we read it
      if (type == AllowedCollision::ALWAYS) {
        always_allow_collision = true;
        if (!cdata->req->verbose)
          logDebug("Collision between '%s' and '%s' is always allowed. No "
                   "contacts are computed.",
  ...
  if (cd1->type == BodyTypes::ROBOT_LINK &&
      cd2->type == BodyTypes::ROBOT_ATTACHED) {
    const std::set<std::string> &tl = cd2->ptr.ab->getTouchLinks();
    if (tl.find(cd1->getID()) != tl.end()) {
      always_allow_collision = true;
      if (!cdata->req->verbose)
        logDebug("Robot link '%s' is allowed to touch attached object '%s'. No "
                 "contacts are computed.",
                 cd1->getID().c_str(), cd2->getID().c_str());
    }
  } else {
    if (cd2->type == BodyTypes::ROBOT_LINK &&
        cd1->type == BodyTypes::ROBOT_ATTACHED) {
      const std::set<std::string> &tl = cd1->ptr.ab->getTouchLinks();
      if (tl.find(cd2->getID()) != tl.end()) {
        always_allow_collision = true;
        if (!cdata->req->verbose)
          logDebug("Robot link '%s' is allowed to touch attached object '%s'. "
  ...
  if (!cdata->req->verbose)
    logDebug("Actually checking collisions between %s and %s",
             cd1->getID().c_str(), cd2->getID().c_str());
  ...
  /*
   * FIXME: The old Distance Query Data structures that existed in this package
   * were replaced by their counterparts in the
   * MoveIt! library.  Unfortunately the legacy code here isn't fully compatible
   * with the new structure types and  so for
   * now the broken code below has been disabled until the discrepancies get
   * amended.
   */
  if (cdata->req->type == DistanceRequestType::GLOBAL) {
    /*      it1 = cdata->res->distance.find(cd1->ptr.obj->id_);
          ...
          if (cdata->req->active_components_only)
          {
            if (active1 && active2)
            {
              if (it1 != cdata->res->distance.end() && it2 !=
       cdata->res->distance.end())
                dist_threshold = std::max(it1->second.min_distance,
       it2->second.min_distance);
            }
  ...
  double d = fcl::distance(
      o1, o2, fcl::DistanceRequest{.enable_nearest_point = true}, fcl_result);
  ...
  if (d < dist_threshold) {
    ...
    dist_result.nearest_points[0] =
        Eigen::Vector3d(fcl_result.nearest_points[0].data.vs);
    dist_result.nearest_points[1] =
        Eigen::Vector3d(fcl_result.nearest_points[1].data.vs);
    ...
    /*
      * FIXME: The old Distance Query Data structures that existed in this
     * package were replaced by their counterparts in the
      * MoveIt! library.  Unfortunately the legacy code here isn't fully
     * compatible with the new structure types and  so for
      * now the broken code below has been disabled until the discrepancies get
     * amended.
      */
    if (cdata->req->type == DistanceRequestType::GLOBAL) {
      ...
      if (active1)
      {
        ...
        {
          cdata->res->distances.insert(std::make_pair(cd1->ptr.obj->id_,
      dist_result));
        }
        else
        {
      ...
      if (active2)
      {
        if (it2 == cdata->res->distance.end())
        {
          cdata->res->distance.insert(std::make_pair(cd2->ptr.obj->id_,
      dist_result));
        }
        else
        {
