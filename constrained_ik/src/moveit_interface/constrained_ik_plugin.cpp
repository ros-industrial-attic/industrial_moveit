...
PLUGINLIB_EXPORT_CLASS(constrained_ik::ConstrainedIKPlugin,
...
bool ConstrainedIKPlugin::initialize(const std::string &robot_description,
  ...
  if (!urdf_model || !srdf) {
    ROS_ERROR_STREAM(
        "URDF and SRDF must be loaded for Constrained Ik solver to work.");
    return false;
  }
  ...
  const robot_model::JointModelGroup *joint_model_group =
      robot_model_ptr_->getJointModelGroup(group_name);
  ...
  try {
    ...
    std::string constraint_param =
        "constrained_ik_solver/" + group_name + "/constraints";
    solver_->addConstraintsFromParamServer(constraint_param);
    solver_->init(
        kin_); // inside try because it has the potential to throw and error
  } catch (exception &e) {
...
bool ConstrainedIKPlugin::getPositionIK(
...
bool ConstrainedIKPlugin::searchPositionIK(
    const geometry_msgs::Pose &ik_pose,
...
bool ConstrainedIKPlugin::getPositionFK(
