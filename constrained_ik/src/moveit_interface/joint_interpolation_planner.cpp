...
bool JointInterpolationPlanner::solve(
...
bool JointInterpolationPlanner::solve(
  ...
  const robot_model::JointModelGroup *group_model =
      rob_model->getJointModelGroup(request_.group_name);
  ...
  ROS_INFO_STREAM("Joint Interpolated Planner will plan for group: "
                  << request_.group_name << " with tip link '"
                  << link_names.back() << "'");
  ...
  if (!request_.goal_constraints[0].joint_constraints.empty()) {
    for (unsigned int i = 0;
      ...
      ROS_DEBUG(
          "Setting joint %s from %f to position %f",
          request_.goal_constraints[0].joint_constraints[i].joint_name.c_str(),
          *start_state.getJointPositions(joint_names[i]),
    ...
    if (!request_.goal_constraints[0].position_constraints.empty() &&
      ...
      pose.orientation =
          request_.goal_constraints[0].orientation_constraints[0].orientation;
               ...
               request_.goal_constraints[0].orientation_constraints.empty()) {
      tf::poseEigenToMsg(start_state.getFrameTransform(link_names.back()),
                         pose);
      pose.position = request_.goal_constraints[0]
                          .position_constraints[0]
                          .constraint_region.primitive_poses[0]
  ...
  if (planning_scene_->isPathValid(*traj, request_.group_name)) {
    ROS_INFO("Joint Interpolated Planner generated a collision-free trajectory "
             "with %i points! :)",
             steps);
...
void JointInterpolationPlanner::setPlannerConfiguration(
    double joint_discretization_step, bool debug_mode) {
  if (joint_discretization_step > 0)
    joint_discretization_step_ = joint_discretization_step;
  else
    ROS_ERROR("Joint Interpolated Planner joint discretization step must be "
              "greater than zero.");
