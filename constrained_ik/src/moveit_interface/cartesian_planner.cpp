...
CartesianPlanner::CartesianPlanner(const std::string &name,
...
bool CartesianPlanner::initializeSolver() {
  ...
  if (!kin.init(robot_model_->getJointModelGroup(getGroupName()))) {
    ROS_ERROR("Cartesian planner could not load solver for move_group %s",
              getGroupName().c_str());
    return false;
  }
...
void CartesianPlanner::setPlannerConfiguration(
    double translational_discretization_step,
    double orientational_discretization_step, bool debug_mode) {
  if (translational_discretization_step > 0)
    translational_discretization_step_ = translational_discretization_step;
  else
    ROS_WARN("Cartesian Planner translational discretization step must be "
             "greater than zero.");

  if (orientational_discretization_step > 0)
    orientational_discretization_step_ = orientational_discretization_step;
  else
    ROS_WARN("Cartesian Planner orientational discretization step must be "
             "greater than zero.");
...
void CartesianPlanner::resetPlannerConfiguration() {
  translational_discretization_step_ =
      DEFAULT_TRANSLATIONAL_DISCRETIZATION_STEP;
  orientational_discretization_step_ =
      DEFAULT_ORIENTATIONAL_DISCRETIZATION_STEP;
  debug_mode_ = false;
}

void CartesianPlanner::setSolverConfiguration(
...
bool CartesianPlanner::solve(planning_interface::MotionPlanResponse &res) {
  ...
  robot_trajectory::RobotTrajectoryPtr traj(
      new robot_trajectory::RobotTrajectory(robot_model_, request_.group_name));
  const robot_model::JointModelGroup *jmg =
      robot_model_->getJointModelGroup(request_.group_name);
  ...
  if (!request_.goal_constraints[0].joint_constraints.empty()) {
    ...
    for (size_t i = 0;
         i < request_.goal_constraints[0].joint_constraints.size(); ++i) {
      goal_joint_map
          [request_.goal_constraints[0].joint_constraints[i].joint_name] =
    ...
    if (!request_.goal_constraints[0].position_constraints.empty() &&
        !request_.goal_constraints[0].orientation_constraints.empty()) {
      pose.position = request_.goal_constraints[0]
                          .position_constraints[0]
                          .constraint_region.primitive_poses[0]
                          .position;
      pose.orientation =
          request_.goal_constraints[0].orientation_constraints[0].orientation;
    } else if (!request_.goal_constraints[0].position_constraints.empty() &&
               request_.goal_constraints[0].orientation_constraints.empty()) {
      tf::poseEigenToMsg(start_state.getGlobalLinkTransform(link_names.back()),
                         pose);
      pose.position = request_.goal_constraints[0]
                          .position_constraints[0]
                          .constraint_region.primitive_poses[0]
  ...
  ROS_DEBUG_NAMED("clik", "Setting Position x from %f to %f",
                  start_pose.translation()(0), goal_pose.translation()(0));
  ROS_DEBUG_NAMED("clik", "Setting Position y from %f to %f",
                  start_pose.translation()(1), goal_pose.translation()(1));
  ROS_DEBUG_NAMED("clik", "Setting Position z from %f to %f",
                  start_pose.translation()(2), goal_pose.translation()(2));
  ROS_DEBUG_NAMED("clik", "Setting Position yaw   from %f to %f",
                  start_pose.rotation().eulerAngles(3, 2, 1)(0),
                  goal_pose.rotation().eulerAngles(3, 2, 1)(0));
  ROS_DEBUG_NAMED("clik", "Setting Position pitch from %f to %f",
                  start_pose.rotation().eulerAngles(3, 2, 1)(1),
                  goal_pose.rotation().eulerAngles(3, 2, 1)(1));
  ROS_DEBUG_NAMED("clik", "Setting Position roll  from %f to %f",
                  start_pose.rotation().eulerAngles(3, 2, 1)(2),
  ...
  bool found_ik;
  mid_state =
      robot_model::RobotStatePtr(new robot_model::RobotState(start_state));
  for (int j = 0; j < steps; j++) {
    if (j != 0) {
      ...
      try {
        found_ik = solver_->calcInvKin(poses[j], start_joints, planning_scene_,
                                       joint_angles);
      ...
      if (!found_ik ||
          planning_scene_->isStateColliding(*mid_state, request_.group_name)) {
        if (!debug_mode_) {
    ...
    if (res.planning_time_ > request_.allowed_planning_time) {
      ROS_INFO(
          "Cartesian planner was unable to find solution in allowed time. :(");
...
CartesianPlanner::interpolateCartesian(const Eigen::Affine3d &start,
  ...
  unsigned steps_translation =
      static_cast<unsigned>(delta_translation.norm() / ds) + 1;
  unsigned steps_rotation =
      static_cast<unsigned>(delta_rotation.angle() / dt) + 1;
