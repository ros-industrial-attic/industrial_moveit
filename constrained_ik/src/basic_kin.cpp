...
bool BasicKin::calcFwdKin(const Eigen::VectorXd &joint_angles,
  ...
  if (joint_angles.size() != chain.getNrOfJoints()) {
    ROS_ERROR_STREAM("Number of joint angles ["
                     << joint_angles.size() << "] must match number of joints ["
                     << chain.getNrOfJoints() << "].");
    return false;
  }
...
bool BasicKin::calcJacobian(const VectorXd &joint_angles,
...
bool BasicKin::checkJoints(const VectorXd &vec) const {
  ...
  for (int i = 0; i < vec.size(); ++i)
    if ((vec[i] < joint_limits_(i, 0)) || (vec(i) > joint_limits_(i, 1))) {
      ROS_ERROR("Joint %d is out-of-range (%g < %g < %g)", i,
                joint_limits_(i, 0), vec(i), joint_limits_(i, 1));
      jnt_bounds_ok = false;
    }
  if (jnt_bounds_ok == false)
    return false;
...
int BasicKin::getJointNum(const std::string &joint_name) const {
  std::vector<std::string>::const_iterator it =
      find(joint_list_.begin(), joint_list_.end(), joint_name);
...
int BasicKin::getLinkNum(const std::string &link_name) const {
  std::vector<std::string>::const_iterator it =
      find(link_list_.begin(), link_list_.end(), link_name);
...
bool BasicKin::init(const moveit::core::JointModelGroup *group) {
  ...
  const boost::shared_ptr<const urdf::ModelInterface> urdf =
      group->getParentModel().getURDF();
...
void BasicKin::KDLToEigen(const KDL::Jacobian &jacobian,
...
bool BasicKin::getSubChain(const std::string link_name,
...
bool BasicKin::linkTransforms(
    const VectorXd &joint_angles, std::vector<KDL::Frame> &poses,
  ...
  for (size_t ii = 0; ii < n; ++ii) {
    ...
    if (fk_solver_->JntToCart(kdl_joints, poses[ii],
                              link_num < 0 ? -1 : link_num + 1) <
        0) /*root=0, link1=1, therefore add +1 to link num*/
    {
...
bool BasicKin::solvePInv(const MatrixXd &A, const VectorXd &b,
                         VectorXd &x) const {
  ...
  if (A.rows() != b.size()) {
    ROS_ERROR("Matrix size mismatch: A(%ld,%ld), b(%ld)", A.rows(), A.cols(),
              b.size());
    return false;
  }

  // Calculate A+ (pseudoinverse of A) = V S+ U*, where U* is Hermition of U
  // (just transpose if all values of U are real)
...
bool BasicKin::dampedPInv(const MatrixXd &A, MatrixXd &P, const double eps,
                          const double lambda) {
  if ((A.rows() == 0) || (A.cols() == 0)) {
