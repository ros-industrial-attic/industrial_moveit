...
const std::vector<std::string> SUPPORTED_COLLISION_DETECTORS = {
    "FCL"}; /**< Supported collision detector */
...
void Constrained_IK::addConstraintsFromParamServer(
    const std::string &parameter_name) {
  XmlRpc::XmlRpcValue constraints_xml;
  boost::shared_ptr<pluginlib::ClassLoader<constrained_ik::Constraint>>
      constraint_loader;

  constraint_loader.reset(
      new pluginlib::ClassLoader<constrained_ik::Constraint>(
          "constrained_ik", "constrained_ik::Constraint"));
  ...
  for (int i = 0; i < constraints_xml.size(); ++i) {
    ...
    if (constraint_xml.hasMember("class") &&
        ...
        constraint_xml["primary"].getType() ==
      ...
      ROS_ERROR(
          "Constraint must have class(string) and primary(boolean) members");
    }
  }
}
...
void Constrained_IK::setSolverConfiguration(
    const ConstrainedIKConfiguration &config) {
  config_ = config;
  validateConstrainedIKConfiguration<ConstrainedIKConfiguration>(config_);
}

constrained_ik::ConstraintResults Constrained_IK::evalConstraint(
    constraint_types::ConstraintTypes constraint_type,
...
Eigen::MatrixXd
Constrained_IK::calcNullspaceProjection(const Eigen::MatrixXd &J) const {
...
Eigen::MatrixXd Constrained_IK::calcNullspaceProjectionTheRightWay(
    const Eigen::MatrixXd &A) const {
  Eigen::JacobiSVD<MatrixXd> svd(A, Eigen::ComputeFullV);
  MatrixXd V(svd.matrixV());

  // determine rank using same algorithym as latest Eigen
  // TODO replace next 10 lines of code with rnk = svd.rank(); once eigen3 is
  // updated
  int rnk = 0;
  if (svd.singularValues().size() == 0) {
    ...
    double threshold =
        std::min(A.rows(), A.cols()) * Eigen::NumTraits<double>::epsilon();
    double premultiplied_threshold = svd.singularValues().coeff(0) * threshold;
    rnk = svd.nonzeroSingularValues() - 1;
    while (rnk >= 0 &&
           svd.singularValues().coeff(rnk) < premultiplied_threshold)
      --rnk;
    rnk++;
  }
...
Eigen::MatrixXd
Constrained_IK::calcDampedPseudoinverse(const Eigen::MatrixXd &J) const {
  MatrixXd J_pinv;
  ...
  if (basic_kin::BasicKin::dampedPInv(J, J_pinv)) {
    ...
    throw std::runtime_error("Not able to calculate damped pseudoinverse!  IK "
...
bool Constrained_IK::calcInvKin(
  ...
  constrained_ik::SolverState state =
      getState(goal, joint_seed); // create state vars for this IK solve
  ...
  if (planning_scene) {
    state.robot_state = robot_state::RobotStatePtr(
        new moveit::core::RobotState(planning_scene->getCurrentState()));

    // Check and make sure the correct collision detector is loaded.
    auto pos = std::find(SUPPORTED_COLLISION_DETECTORS.begin(),
                         SUPPORTED_COLLISION_DETECTORS.end(),
                         planning_scene->getActiveCollisionDetectorName());
    if (pos == SUPPORTED_COLLISION_DETECTORS.end()) {
      std::stringstream error_message;
      error_message
          << " Constrained IK requires the use of collision detectors: ";
      for (auto &d : SUPPORTED_COLLISION_DETECTORS) {
        error_message << "'" << d << "' ";
      }
      error_message << ".\nSet or add the 'collision_detector' parameter to an "
                       "allowed collision detector in the move_group.launch "
                       "file"
                    << std::endl;
  ...
  if (state.condition == initialization_state::NothingInitialized ||
      state.condition == initialization_state::AuxiliaryOnly)
    throw std::runtime_error("Must call init() before using Constrained_IK and "
  ...
  while (true) {
    ...
    // calculate a Jacobian (relating joint-space updates/deltas to
    // cartesian-space errors/deltas)
    // and the associated cartesian-space error/delta vector
    // Primary Constraints
    constrained_ik::ConstraintResults primary =
        evalConstraint(constraint_types::Primary, state);
    // TODO since we already have J_p = USV, use that to get null-projection
    // too.
    // otherwise, we are repeating the expensive calculation of the SVD

    VectorXd dJoint_p;
    dJoint_p.setZero(joint_seed.size());
    if (!primary.isEmpty()) // This is required because not all constraints
                            // always return data.
    {
      ...
      if (config_.allow_primary_normalization &&
          dJoint_norm >
              config_.primary_norm) // limit maximum update radian/meter
      {
    ...
    if (state.condition == initialization_state::PrimaryAndAuxiliary) {
      if (!((config_.limit_auxiliary_motion &&
             state.auxiliary_sum >= config_.auxiliary_max_motion) ||
            (config_.limit_auxiliary_interations &&
             state.iter > config_.auxiliary_max_iterations))) {
        auxiliary = evalConstraint(constraint_types::Auxiliary, state);
        if (!auxiliary.isEmpty()) // This is required because not all
                                  // constraints always return data.
        {
          ...
          dJoint_a = config_.auxiliary_gain * Jnull_a *
                     (auxiliary.error - auxiliary.jacobian * dJoint_p);
          dJoint_norm = dJoint_a.norm();
          if (config_.allow_auxiliary_nomalization &&
              dJoint_norm >
                  config_.auxiliary_norm) // limit maximum update radian/meter
          {
    ...
    if (status == Converged) {
      ROS_DEBUG_STREAM("Found IK solution in " << state.iter << " iterations: "
                                               << joint_angles.transpose());
      return true;
...
SolverStatus Constrained_IK::checkStatus(
  ...
  if (config_.allow_joint_convergence) {
    if (state.joints_delta.cwiseAbs().maxCoeff() <
        config_.joint_convergence_tol) {
      if (state.iter > config_.solver_min_iterations) {
        ROS_DEBUG_STREAM("Joint convergence reached "
                         << state.iter << " / " << config_.solver_max_iterations
  ...
  if (state.iter > config_.solver_max_iterations ||
      (config_.limit_primary_motion &&
       state.primary_sum >= config_.primary_max_motion)) {
    if (!primary.status) {
      if (config_.limit_primary_motion &&
          state.primary_sum >= config_.primary_max_motion) {
        ROS_WARN_STREAM(
            "Primary reached max allowed motion, no solution returned.");
      } else {
        ROS_WARN_STREAM(
            "Solver reached max allowed iteration, no solution returned.");
      }
      return Failed;
    } else if (state.condition == initialization_state::PrimaryAndAuxiliary) {
      ROS_WARN_STREAM("Maximum iterations reached but primary converged so "
                      "returning solution.");
      return Converged;
    }
  }
...
void Constrained_IK::clipToJointLimits(Eigen::VectorXd &joints) const {
  ...
  if (joints.size() != limits.rows())
    throw std::invalid_argument(
        "clipToJointLimits: Unexpected number of joints");
...
void Constrained_IK::init(const basic_kin::BasicKin &kin) {
  if (!kin.checkInitialized())
    throw std::invalid_argument(
        "Input argument 'BasicKin' must be initialized");
...
void Constrained_IK::updateState(constrained_ik::SolverState &state,
  ...
  if (state.planning_scene && state.robot_state) {
    state.robot_state->setJointGroupPositions(
        kin_.getJointModelGroup()->getName(), joints);
