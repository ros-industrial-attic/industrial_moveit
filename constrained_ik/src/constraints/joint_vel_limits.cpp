...
#include <ros/ros.h>
PLUGINLIB_EXPORT_CLASS(constrained_ik::constraints::JointVelLimits,
                       constrained_ik::Constraint)

const double DEFAULT_WEIGHT = 1.0; /**< Default weight */
const double DEFAULT_TIMESTEP =
    0.1; /**< Default timestep used to calculate joint velocities */
...
JointVelLimits::JointVelLimits()
    : Constraint(), weight_(DEFAULT_WEIGHT), timestep_(DEFAULT_TIMESTEP) {}

constrained_ik::ConstraintResults
JointVelLimits::evalConstraint(const SolverState &state) const {
...
Eigen::VectorXd JointVelLimits::calcError(
    const JointVelLimits::JointVelLimitsData &cdata) const {
  ...
  for (int ii = 0; ii < nRows; ++ii) {
    ...
    int velSign =
        cdata.jvel_(jntIdx) < 0 ? 1 : -1; // negative jvel requires positive
                                          // velocity correction, and vice versa
...
void JointVelLimits::loadParameters(const XmlRpc::XmlRpcValue &constraint_xml) {
  ...
  if (!getParam(local_xml, "weights", weight_)) {
    ROS_WARN("Avoid Joint Velocity Limits: Unable to retrieve weights member, "
             "default parameter will be used.");
  }

  if (!getParam(local_xml, "timestep", timestep_)) {
    ROS_WARN("Avoid Joint Velocity Limits: Unable to retrieve timestep member, "
             "default parameter will be used.");
  }

  if (!getParam(local_xml, "debug", debug_)) {
    ROS_WARN("Avoid Joint Velocity Limits: Unable to retrieve debug member, "
             "default parameter will be used.");
  }
}
...
JointVelLimits::JointVelLimitsData::JointVelLimitsData(
    const SolverState &state, const constraints::JointVelLimits *parent)
  ...
  if (parent_->debug_ && !limited_joints_.empty())
    ROS_ERROR_STREAM(limited_joints_.size() << " joint vel limits active: "
                                            << limited_joints_);
}
