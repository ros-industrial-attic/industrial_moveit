...
PLUGINLIB_EXPORT_CLASS(constrained_ik::constraints::GoalZeroJVel,
...
GoalZeroJVel::evalConstraint(const SolverState &state) const {
...
Eigen::VectorXd
GoalZeroJVel::calcError(const GoalZeroJVel::ConstraintData &cdata) const {
  return VectorXd::Zero(numJoints());
}

Eigen::MatrixXd
GoalZeroJVel::calcJacobian(const GoalZeroJVel::ConstraintData &cdata) const {
...
void GoalZeroJVel::loadParameters(const XmlRpc::XmlRpcValue &constraint_xml) {
  ...
  if (!getParam(local_xml, "weights", weight_)) {
    ROS_WARN("Goal Joint Zero Velocity: Unable to retrieve weights member, "
             "default parameter will be used.");
  }

  if (!getParam(local_xml, "debug", debug_)) {
    ROS_WARN("Goal Joint Zero Velocity: Unable to retrieve debug member, "
             "default parameter will be used.");
  }
}
