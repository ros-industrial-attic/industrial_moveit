...
#include <ros/ros.h>
PLUGINLIB_EXPORT_CLASS(constrained_ik::constraints::GoalOrientation,
...
GoalOrientation::GoalOrientation()
    : Constraint(), rot_err_tol_(DEFAULT_ORIENTATION_TOLERANCE),
      weight_(Vector3d::Ones()) {}

constrained_ik::ConstraintResults
GoalOrientation::evalConstraint(const SolverState &state) const {
...
Eigen::Vector3d GoalOrientation::calcAngleError(const Eigen::Affine3d &p1,
  ...
  double theta = Constrained_IK::rangedAngle(
      r12.angle()); // TODO: move rangedAngle to utils class
  return p1.rotation() * r12.axis() *
         theta; // axis k * theta expressed in frame0
}

Eigen::VectorXd GoalOrientation::calcError(
    const GoalOrientation::GoalOrientationData &cdata) const {
...
bool GoalOrientation::checkStatus(
    const GoalOrientation::GoalOrientationData &cdata) const {
...
void GoalOrientation::loadParameters(
  ...
  if (!getParam(local_xml, "orientation_tolerance", rot_err_tol_)) {
    ROS_WARN("Gool Orientation: Unable to retrieve orientation_tolerance "
  ...
  if (getParam(local_xml, "weights", weights)) {
    if (weights.size() == 3) {
      ...
      ROS_WARN("Gool Orientation: Unable to add weights member, value must be "
               "a array of size 3.");
    }
  } else {
    ROS_WARN("Gool Orientation: Unable to retrieve weights member, default "
             "parameter will be used.");
  }

  if (!getParam(local_xml, "debug", debug_)) {
    ROS_WARN("Gool Orientation: Unable to retrieve debug member, default "
             "parameter will be used.");
  }
}

GoalOrientation::GoalOrientationData::GoalOrientationData(
