...
#include <ros/ros.h>
PLUGINLIB_EXPORT_CLASS(constrained_ik::constraints::GoalToolOrientation,
...
GoalToolOrientation::evalConstraint(const SolverState &state) const {
...
Eigen::VectorXd GoalToolOrientation::calcError(
  ...
  Vector3d err = cdata.state_.pose_estimate.rotation().transpose() *
                 calcAngleError(cdata.state_.pose_estimate, cdata.state_.goal);
...
Eigen::MatrixXd GoalToolOrientation::calcJacobian(
  ...
  // rotate jacobian into tool frame by premultiplying by otR.transpose()
  MatrixXd J =
      cdata.state_.pose_estimate.rotation().transpose() * tmpJ.bottomRows(3);
