...
PLUGINLIB_EXPORT_CLASS(constrained_ik::constraints::GoalPosition,
...
GoalPosition::evalConstraint(const SolverState &state) const {
...
Eigen::VectorXd
GoalPosition::calcError(const GoalPosition::GoalPositionData &cdata) const {
...
Eigen::MatrixXd
GoalPosition::calcJacobian(const GoalPosition::GoalPositionData &cdata) const {
  MatrixXd tmpJ;
...
bool GoalPosition::checkStatus(
    const GoalPosition::GoalPositionData &cdata) const {
...
void GoalPosition::loadParameters(const XmlRpc::XmlRpcValue &constraint_xml) {
  ...
  if (getParam(local_xml, "position_tolerance", pos_tol)) {
    ...
    ROS_WARN("Goal Position: Unable to retrieve position_tolerance member, "
  ...
  if (getParam(local_xml, "weights", weights)) {
    if (weights.size() == 3) {
      ...
      ROS_WARN("Gool Position: Unable to add weights member, value must be a "
               "array of size 3.");
    }
  } else {
    ROS_WARN("Gool Position: Unable to retrieve weights member, default "
  ...
  if (getParam(local_xml, "debug", debug)) {
    ...
    ROS_WARN("Goal Position: Unable to retrieve debug member, default "
             "parameter will be used.");
  }
}

GoalPosition::GoalPositionData::GoalPositionData(const SolverState &state)
