 ...
 */
static planning_scene::PlanningScenePtr
getExampleRobotData(robot_model_loader::RobotModelLoaderPtr &loader) {
  ros::NodeHandle nh;
  std::string urdf_file_path, srdf_file_path;

  urdf_file_path =
      ros::package::getPath("constrained_ik") + "/test/resources/ur10.urdf";
  srdf_file_path =
      ros::package::getPath("constrained_ik") + "/test/resources/ur10.srdf";

  std::ifstream ifs1(urdf_file_path.c_str());
  std::string urdf_string((std::istreambuf_iterator<char>(ifs1)),
                          (std::istreambuf_iterator<char>()));

  std::ifstream ifs2(srdf_file_path.c_str());
  std::string srdf_string((std::istreambuf_iterator<char>(ifs2)),
                          (std::istreambuf_iterator<char>()));
  ...
  planning_scene::PlanningScenePtr planning_scene(
      new planning_scene::PlanningScene(loader->getModel()));
 ...
 */
static bool evaluteExampleIK(constrained_ik::basic_kin::BasicKin &kin,
                             constrained_ik::Constrained_IK &ik,
                             planning_scene::PlanningScenePtr planning_scene =
  ...
  Eigen::VectorXd goal =
      Eigen::VectorXd::Map(goal_vector.data(), goal_vector.size());
  Eigen::VectorXd seed =
      Eigen::VectorXd::Map(seed_vector.data(), seed_vector.size());
  ...
  if (ik.calcInvKin(goalPose, seed, planning_scene, joints)) {
    ...
    ROS_INFO_STREAM(
        "Desired Joint State: " << goal.transpose().format(vector_fmt));
    ROS_INFO_STREAM(
        "Seed Joint State:    " << seed.transpose().format(vector_fmt));
    ROS_INFO_STREAM(
        "Final Joint State:   " << joints.transpose().format(vector_fmt));
