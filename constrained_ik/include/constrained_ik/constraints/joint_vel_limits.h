...
public:
  ...
  struct JointVelLimitsData : public ConstraintData {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    const constraints::JointVelLimits
    ...
    JointVelLimitsData(const constrained_ik::SolverState &state,
                       const constraints::JointVelLimits *parent);
  };
  ...
  /**
   * @brief Creates vector representing velocity error term corresponding to
   * calcJacobian()
  ...
  virtual bool checkStatus(const JointVelLimitsData &cdata) const {
    return cdata.limited_joints_.size() == 0;
  }
