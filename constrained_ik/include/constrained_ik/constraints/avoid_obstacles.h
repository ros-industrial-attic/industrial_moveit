...
/** @brief Contains distance information in the planning frame queried from
 * getDistanceInfo() */
struct DistanceInfo {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  std::
      string nearest_obsticle; /**< The link name for nearest obsticle/link to
                                  request link. */
  Eigen::Vector3d link_point;  /**< Point on request link */
  Eigen::Vector3d
      obsticle_point; /**< Point on nearest link to requested link */
  Eigen::Vector3d
      avoidance_vector; /**< Normilized Vector created by nearest points */
 ...
 * @param distance_info_map Stores the distance information for each link in
 * DistanceDetailedMap
 * @param tf This allows for a transformation to be applied the distance data
 * since it is always returned in the world frame from fcl.
 * @return bool, true if succesfully converted DistanceDetailedMap to
 * DistanceInfoMap
 */
...
protected:
  ...
  struct LinkAvoidance {
    ...
    double
        weight_; /**< importance weight applied to this avoidance constraint */
    double
        min_distance_; /**< minimum obstacle distance allowed for convergence */
    double avoidance_distance_; /**< distance at which to start avoiding the
                                   obstacle */
    ...
    int num_obstacle_joints_; /**< number of joints inboard to the obstacle link
                                 */
    std::string
        link_name_; /**< the name of the link that is to avoid obstacles */
    KDL::Chain
        avoid_chain_;        /**< the kinematic chain from base to the obstacle
                                avoidance link */
    int num_inboard_joints_; /**< number of joints in the inboard chain */
    KDL::Vector
        link_point_; /**< vector to point on link closest to an obstacle */
    KDL::ChainJntToJacSolver
        *jac_solver_; /**< a KDL object for computing jacobians */
    KDL::Vector
        obstacle_point_; /**< vector to point on link closest to an obstacle */
  };

  std::map<std::string, LinkAvoidance>
      links_; /**< @brief map from link name to its avoidance data */
  std::vector<std::string>
      link_names_; /**< @brief list of links that should avoid obstacles */
  std::
      set<const robot_model::LinkModel *>
          link_models_; /**< @brief a set of LinkModel for each link in
                           link_names_ */
  double
      distance_threshold_; /**< @brief a distance threshold used to speed up
                              distance queries */
  ...
  LinkAvoidance *getLinkData(const std::string link_name) {
    ...
    if (it != links_.end()) {
      ...
      ROS_WARN_STREAM(
          "Failed to retrieve avoidance data for link: " << link_name);
      return NULL;
    }
  }
...
public:
  ...
  struct AvoidObstaclesData : public ConstraintData {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    const constraints::AvoidObstacles
        *parent_; /**< pointer to parent class AvoidObstacles */
    collision_detection::DistanceResult
        distance_res_; /**< stores the minimum distance results */
    collision_detection::DistanceMap
        distance_map_; /**< map of link names to its distance results */
    DistanceInfoMap
        distance_info_map_; /**< map of link names to distance information */

    /** @brief See base class for documentation */
    AvoidObstaclesData(const constrained_ik::SolverState &state,
                       const constraints::AvoidObstacles *parent);
  };
  ...
  /**
   * @brief Creates Jacobian for avoiding a collision with link closest to a
   * collision
   * @param cdata The constraint specific data.
   * @param link The link to process
   * @return Jacobian scaled by weight
   */
  virtual Eigen::MatrixXd calcJacobian(const AvoidObstaclesData &cdata,
   ...
   * @param cdata The constraint specific data.
   * @param link The link to process
   * @return VectorXd of joint velocities for obstacle avoidance scaled by
   * weight
   */
  virtual Eigen::VectorXd calcError(const AvoidObstaclesData &cdata,
                                    const LinkAvoidance &link) const;
   ...
   * @return True
   */
  virtual bool checkStatus(const AvoidObstaclesData &cdata,
                           const LinkAvoidance &link) const;
   ...
   */
  virtual void setMinDistance(const std::string &link_name,
   ...
   */
  virtual void setAmplitude(const std::string &link_name,
   ...
   */
  virtual void setAvoidanceDistance(const std::string &link_name,
  ...
  virtual void setAvoidanceLinks(const std::vector<std::string> &link_names) {
    ...
    for (std::vector<std::string>::const_iterator it = link_names.begin();
         it < link_names.end(); ++it) {
  ...
  virtual void addAvoidanceLink(const std::string &link_name) {
    if (std::find(link_names_.begin(), link_names_.end(), link_name) ==
        link_names_.end()) {
  ...
  virtual void updateDistanceThreshold() {
    ...
    for (std::map<std::string, LinkAvoidance>::const_iterator it =
             links_.begin();
         it != links_.end(); it++) {
