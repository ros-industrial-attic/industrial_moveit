...
public:
   ...
   * (including uninitialized BasicKin)
   */
  bool calcFwdKin(const Eigen::VectorXd &joint_angles, const std::string &base,
                  const std::string &tip, KDL::Frame &pose) const;
   ...
   * (including uninitialized BasicKin)
   */
  bool calcJacobian(const Eigen::VectorXd &joint_angles,
                    Eigen::MatrixXd &jacobian) const;

  /**
   * @brief Checks if BasicKin is initialized (init() has been run: urdf model
   * loaded, etc.)
   ...
   * @return True if size of vec matches # of robot joints and all joints are
   * within limits
   */
   ...
   * @param names Output vector of joint names, copied from joint_list_ created
   * in init()
   ...
   * @param names Output vector of names, copied from link_list_ created in
   * init()
   * @return True if BasicKin has been successfully initialized
   */
   ...
   * Creates KDL::Chain from urdf::Model, populates joint_list_, joint_limits_,
   * and link_list_
  ...
  /**
   * @brief Calculates transforms of each link relative to base (not including
   * base)
   * If link_names is specified, only listed links will be returned. Otherwise
   * all links in link_list_ will be returned
   ...
   */
  bool solvePInv(const Eigen::MatrixXd &A, const Eigen::VectorXd &b,
   ...
   */
  static void KDLToEigen(const KDL::Jacobian &jacobian,
...
private:
  ...
  boost::scoped_ptr<KDL::ChainFkSolverPos_recursive>
      fk_solver_; /**< KDL Forward Kinematic Solver */
  boost::scoped_ptr<KDL::ChainJntToJacSolver>
      jac_solver_; /**< KDL Jacobian Solver */
