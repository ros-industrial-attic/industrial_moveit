...
struct ConstrainedIKConfiguration {
  ...
  bool
      allow_joint_convergence; /**< Allow the solver to converge based on joint
                                  convergence. */
  bool allow_primary_normalization;  /**< Allow the solver to normalize primary
                                        motion. */
  bool allow_auxiliary_nomalization; /**< Allow the solver to normalize
                                        auxiliary motion. */
  bool
      limit_primary_motion; /**< Allow the solver to limit the primary motion.
                               */
  bool limit_auxiliary_motion; /**< Allow the solver to limit the auxiliary
                                  motion. */
  bool
      limit_auxiliary_interations; /**< Allow the solver to limit the number of
                                      auxiliary interation. */
  int solver_max_iterations; /**< Solver's maximum number of iterations allowed.
                                */
  int solver_min_iterations; /**< Solver's minimum number of iterations
                                required. */
  int auxiliary_max_iterations; /**< Auxiliary's maximum number of iteration
                                   allowed. */
...
/**
 * @brief Convert ConstrainedIKDynamicReconfigureConfig to
 ...
 */
ConstrainedIKConfiguration
convertToConstrainedIKConfiguration(CLIKDynamicConfig &config);

template <typename T> void validateConstrainedIKConfiguration(T &config) {
  if (config.limit_auxiliary_motion) {
    if (config.auxiliary_norm > config.auxiliary_max_motion) {
      ...
      unsigned int divisor =
          floor(config.auxiliary_max_motion / config.auxiliary_norm) + 1;
 ...
 */
bool getParam(XmlRpc::XmlRpcValue &config, const std::string &key,
              double &value);
