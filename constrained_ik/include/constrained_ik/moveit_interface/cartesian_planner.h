...
public:
  ...
  CartesianPlanner(const CartesianPlanner &other)
      : planning_interface::PlanningContext(other), terminate_(false),
        robot_description_(robot_description_), solver_(solver_) {}
   ...
   * @param translational_discretization_step Max translational discretization
   * step
   * @param orientational_discretization_step Max orientational discretization
   * step
   * @param debug_mode Set debug state
   */
  void setPlannerConfiguration(double translational_discretization_step,
                               double orientational_discretization_step,
...
private:
  /**
   * @brief Preform position and orientation interpolation between start and
   * stop.
  ...
  double translational_discretization_step_; /**< Max translational
                                                discretization step */
  double orientational_discretization_step_; /**< Max orientational
                                                discretization step */
  bool debug_mode_;                          /**< Debug state */
  boost::atomic<bool> terminate_;            /**< Termination flag */
  std::string
      robot_description_; /**< robot description value from ros param server */
