...
public:
  ...
  CLIKPlannerManager() : planning_interface::PlannerManager(), nh_("~") {}
  ...
  /** @brief See base clase for documentation */
  virtual void
  managerDynamicReconfigureCallback(CLIKPlannerDynamicConfig &config,
                                    uint32_t level);

  /**
   * @brief A callback function for the cartesian planners solver dynamic
   * reconfigure server
   ...
   */
  virtual void cartesianDynamicReconfigureCallback(CLIKDynamicConfig &config,
                                                   uint32_t level,
                                                   std::string group_name);

protected:
  typedef dynamic_reconfigure::Server<CLIKPlannerDynamicConfig>
      ManagerDynReconfigServer; /**< Type definition for the planning manager
                                   dynamic reconfigure server */
  typedef dynamic_reconfigure::
      Server<CLIKDynamicConfig>
          CartesianDynReconfigServer; /**< Type definition for the cartesian
                                         planner dynamic reconfigure server */
  typedef boost::
      shared_ptr<ManagerDynReconfigServer>
          ManagerDynReconfigServerPtr; /**< Type definition for the planning
                                          manager dynamic reconfigure server
                                          shared pointer*/
  typedef boost::shared_ptr<CartesianDynReconfigServer>
      CartesianDynReconfigServerPtr; /**< Type definition for the cartesian
                                        planner dynamic reconfigure server
                                        shared pointer*/
  ...
  ManagerDynReconfigServerPtr
      dynamic_reconfigure_server_; /**< Planner dynamic reconfigure server
                                      object */
  std::map<std::string, CartesianDynReconfigServerPtr>
      cartesian_dynamic_reconfigure_server_; /**< Cartesian constrianed ik
                                                solver dynamic reconfigure
                                                server */
  std::map<std::pair<std::string, std::string>,
           planning_interface::PlanningContextPtr>
      planners_; /**< Containes all the availble CLIK planners */
};
} // namespace constrained_ik
