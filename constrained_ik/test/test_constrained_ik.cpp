...
TEST_F(BasicIKTest, inputValidation) {
  ...
  EXPECT_EQ(Constrained_IK().checkInitialized(),
            constrained_ik::initialization_state::NothingInitialized);
  constrained_ik::Constraint *goal_ptr =
      new constrained_ik::constraints::GoalPosition();
  ik.addConstraint(goal_ptr, constrained_ik::constraint_types::Primary);
  EXPECT_NE(ik.checkInitialized(),
            constrained_ik::initialization_state::NothingInitialized);
}

/** @brief This performs input validation test for the Constrained_IK calcInvKin
 * function */
TEST_F(BasicIKTest, calcInvKinInputValidation) {
  ...
  EXPECT_ANY_THROW(
      ik.calcInvKin(homePose, VectorXd::Zero(99), joints)); // wrong-sized Seed

  // valid input
  //   - GTest doesn't print exception info.  This method allows us to print
  //   that info, for easier troubleshooting
  try {
...
TEST_F(BasicIKTest, knownPoses) {
  ...
  seed = expected;
  EXPECT_TRUE(
      kin.calcFwdKin(expected, pose)); // expect the pose to remain unchanged
  ...
  constrained_ik::Constraint *goal_pose_ptr =
      new constrained_ik::constraints::GoalPose();
  ...
  std::cout << "Testing seed vector "
            << VectorXd::Zero(expected.size()).transpose() << std::endl
  ...
  std::cout << "Testing seed vector "
            << VectorXd::Zero(expected.size()).transpose() << std::endl
            << "*very far* from expected: " << expected.transpose()
            << std::endl;
  seed = VectorXd::Zero(expected.size());
  EXPECT_TRUE(ik.calcInvKin(pose, seed, joints));
  EXPECT_TRUE(kin.calcFwdKin(joints, rslt_pose));
...
TEST_F(BasicIKTest, NullMotion) {
  ...
  // adding primary constraints to move to a position, and auxillary constraints
  // to move to pose
  ik.clearConstraintList();
  constrained_ik::Constraint *goal_position_ptr =
      new constrained_ik::constraints::GoalPosition();
  ik.addConstraint(goal_position_ptr,
                   constrained_ik::constraint_types::Primary);
  constrained_ik::Constraint *goal_orientation_ptr =
      new constrained_ik::constraints::GoalOrientation();
  ik.addConstraint(goal_orientation_ptr,
                   constrained_ik::constraint_types::Auxiliary);
  ...
  for (int j = 0; j < 20; j++) // do 3 random examples of far starts
  {
    for (int i = 0; i < 6; i++) // find a random pose
    {
      boost::random::uniform_int_distribution<int> angle_degrees(-180 + 50,
                                                                 180 - 50);
    ...
    if (rslt_pose.rotation().isApprox(pose.rotation(), 9e-3) &&
        rslt_pose.translation().isApprox(pose.translation(), 1e-3))
      num_success++;
  }
  ...
  for (int j = 0; j < 20; j++) // do 20 random examples of farther starts
  {
    for (int i = 0; i < 6; i++) // find a random pose
    {
      boost::random::uniform_int_distribution<int> angle_degrees(-180 + 90,
                                                                 180 - 90);
...
TEST_F(BasicIKTest, NullMotionPose) {
  ...
  // adding primary constraints to move to an orientation, and auxillary
  // constraints to move to position
  ik.clearConstraintList();
  constrained_ik::Constraint *goal_orientation_ptr =
      new constrained_ik::constraints::GoalOrientation();
  ik.addConstraint(goal_orientation_ptr,
...
TEST_F(BasicIKTest, obstacleAvoidanceAuxiliaryConstraint) {
  ...
  // adding primary constraints to move to an orientation, and Auxiliary
  // constraints to move to position
  ik.clearConstraintList();
  constrained_ik::Constraint *goal_position_ptr =
      new constrained_ik::constraints::GoalPosition();
  ...
  constrained_ik::constraints::AvoidObstacles *avoid_obstacles_ptr =
      new constrained_ik::constraints::AvoidObstacles();
  ...
  ik.addConstraint(goal_position_ptr,
                   constrained_ik::constraint_types::Primary);
  ik.addConstraint(avoid_obstacles_ptr,
                   constrained_ik::constraint_types::Auxiliary);

  // perform inverse kinematics but the position should not change, just the
  // orientation
  expected << M_PI_2, -M_PI_2, -M_PI_2, -M_PI_2, M_PI_2, -M_PI_2;
  kin.calcFwdKin(expected, pose);
  seed = expected;
...
/**
 * @brief This test checks the consistancy of the axisAngle calculations from a
 * quaternion value
 * Namely does it always return an angle in +/-pi range? (The answer should be
 * yes)
 */
TEST_F(BasicIKTest, consistancy) {
  ...
  for (double theta = -7.0; theta < 7.0; theta += 1.0) {
    ...
    expected << k, ik.rangedAngle(theta);
    ...
    Eigen::Vector4d rslt;
    rslt << aa_q.axis(), aa_q.angle();
    rslt(3) = ik.rangedAngle(aa_q.angle());

    // if theta ~= 0, angleAxis/Quaternion operations will give [1,0,0] as
    // vector
    if (fabs(theta) < 1e-6) {
      expected(0) = 1;
      expected(1) = 0;
    ...
    EXPECT_TRUE(rslt.isApprox(expected, 1e-5) or
                rslt.isApprox(-expected, 1e-5));
  }
}
/** @brief This executes all tests for the Constraine_IK Class and its
 * constraints */
