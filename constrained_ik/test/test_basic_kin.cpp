 ...
 * @brief Test Fixtures
 *
 * Consolidate variable-definitions and init functions for use by multiple
 * tests.
 *
 * @author dsolomon
 * @date Sep 23, 2013
...
const std::string GROUP_NAME =
    "manipulator"; /**< Default group name for tests */
...
protected:
  robot_model_loader::RobotModelLoaderPtr
      loader_;                              /**< Used to load the robot model */
  ...
  virtual void SetUp() {
    ...
    loader_.reset(
        new robot_model_loader::RobotModelLoader(ROBOT_DESCRIPTION_PARAM));
   ...
   * false
   */
  bool comparePoses(const std::vector<KDL::Frame> &actual,
...
/** @brief This performs input validation test for the BasicKin linkTransforms
 * function */
TEST_F(RobotTest, linkTransformsInputValidation) {
  std::vector<std::string> link_names =
      boost::assign::list_of("shoulder_link")("upper_arm_link")("forearm_link")(
          "wrist_1_link")("wrist_2_link")("wrist_3_link");
  std::vector<KDL::Frame> poses;
  int n_links = link_names.size();

  EXPECT_FALSE(BasicKin().linkTransforms(
      VectorXd(), poses)); // un-init BasicKin, names, & Jnts
  EXPECT_FALSE(BasicKin().linkTransforms(
      VectorXd(), poses,
      std::vector<std::string>())); // un-init BasicKin, names, & Jnts
  EXPECT_FALSE(BasicKin().linkTransforms(
      VectorXd(), poses, link_names)); // un-init BasicKin & Jnts
  EXPECT_FALSE(BasicKin().linkTransforms(
      VectorXd::Zero(6), poses,
      std::vector<std::string>())); // un-init BasicKin & names
  EXPECT_FALSE(BasicKin().linkTransforms(VectorXd::Zero(6), poses,
                                         link_names)); // un-init BasicKin

  EXPECT_FALSE(kin.linkTransforms(
      VectorXd(), poses, std::vector<std::string>())); // empty names & joints
  EXPECT_FALSE(kin.linkTransforms(VectorXd::Zero(99), poses,
                                  link_names)); // too many joints
  EXPECT_FALSE(kin.linkTransforms(VectorXd::Constant(6, 1e10), poses,
                                  link_names)); // joints out-of-range

  EXPECT_TRUE(
      kin.linkTransforms(VectorXd::Zero(6), poses, link_names)); // valid input
  EXPECT_TRUE(kin.linkTransforms(VectorXd::Zero(6), poses));     // valid input

  std::vector<std::string> link_names_short =
      boost::assign::list_of(link_names[5]);
  EXPECT_TRUE(kin.linkTransforms(VectorXd::Zero(6), poses,
                                 link_names_short)); // valid short input

  link_names[5] = "fail_on_this";
  EXPECT_FALSE(kin.linkTransforms(VectorXd::Zero(6), poses,
                                  link_names)); // invalid link list
  link_names_short[0] = link_names[5];
  EXPECT_FALSE(kin.linkTransforms(
      VectorXd::Zero(6), poses, link_names_short)); // invalid & short link list
}
...
TEST_F(RobotTest, linkTransformsKnownPoses) {
  ...
  expected[0] = Frame(KDL::Rotation::Quaternion(0.0, 0.0, 0.0, 1.0),
                      Vector(0.000000, 0.000000, 0.127300));
  expected[1] = Frame(KDL::Rotation::Quaternion(0.0, 0.707107, 0.0, 0.707107),
                      Vector(0.000000, 0.220941, 0.127300));
  expected[2] = Frame(KDL::Rotation::Quaternion(0.0, 0.707107, 0.0, 0.707107),
                      Vector(0.612000, 0.049041, 0.127300));
  expected[3] = Frame(KDL::Rotation::Quaternion(0.0, 1.0, 0.0, 0.0),
                      Vector(1.184300, 0.049041, 0.127300));
  expected[4] = Frame(KDL::Rotation::Quaternion(0.0, 1.0, 0.0, 0.0),
                      Vector(1.184300, 0.163941, 0.127300));
  expected[5] = Frame(KDL::Rotation::Quaternion(0.0, 1.0, 0.0, 0.0),
                      Vector(1.184300, 0.163941, 0.011600));
  kin.linkTransforms(joint_angles, actual, link_names);

  EXPECT_TRUE(
      kin.linkTransforms(joint_angles, actual, link_names)); // all link names
  EXPECT_TRUE(comparePoses(actual, expected, 1e-4));
  EXPECT_TRUE(kin.linkTransforms(joint_angles,
                                 actual)); // no link names (defaults to all)
  EXPECT_GT(actual.size(), expected.size());
  actual.pop_back(); // for comparison remove ee_link which is not in list of
                     // test links.
  EXPECT_TRUE(comparePoses(actual, expected, 1e-4));

  // 90,0,0,0,0,0
  joint_angles(0) = M_PI_2;
  expected[0] = Frame(KDL::Rotation::Quaternion(0.0, 0.0, 0.707107, 0.707107),
                      Vector(0.000000, 0.000000, 0.127300));
  expected[1] = Frame(KDL::Rotation::Quaternion(-0.5, 0.5, 0.5, 0.5),
                      Vector(-0.220941, 0.000000, 0.127300));
  expected[2] = Frame(KDL::Rotation::Quaternion(-0.5, 0.5, 0.5, 0.5),
                      Vector(-0.049041, 0.612000, 0.127300));
  expected[3] = Frame(KDL::Rotation::Quaternion(-0.707107, 0.707107, 0.0, 0.0),
                      Vector(-0.049041, 1.184300, 0.127300));
  expected[4] = Frame(KDL::Rotation::Quaternion(-0.707107, 0.707107, 0.0, 0.0),
                      Vector(-0.163941, 1.184300, 0.127300));
  expected[5] = Frame(KDL::Rotation::Quaternion(-0.707107, 0.707107, 0.0, 0.0),
                      Vector(-0.163941, 1.184300, 0.011600));
  kin.linkTransforms(joint_angles, actual, link_names);
  ...
  expected[0] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.000000, 0.000000, 1.000000),
            Vector(0.000000, 0.000000, 0.127300));
  expected[1] =
      Frame(KDL::Rotation::Quaternion(0.000000, -0.000000, 0.000000, 1.000000),
            Vector(0.000000, 0.220941, 0.127300));
  expected[2] =
      Frame(KDL::Rotation::Quaternion(0.000000, -0.000000, 0.000000, 1.000000),
            Vector(-0.000000, 0.049041, 0.739300));
  expected[3] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.707107, 0.000000, 0.707107),
            Vector(-0.000000, 0.049041, 1.311600));
  expected[4] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.707107, 0.000000, 0.707107),
            Vector(-0.000000, 0.163941, 1.311600));
  expected[5] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.707107, 0.000000, 0.707107),
            Vector(0.115700, 0.163941, 1.311600));
  kin.linkTransforms(joint_angles, actual, link_names);
  EXPECT_TRUE(
      kin.linkTransforms(joint_angles, actual, link_names)); // all link names
  EXPECT_TRUE(comparePoses(actual, expected, 1e-4));
  ...
  // joints 2-5
  joint_angles = VectorXd::Zero(6);
  std::vector<std::string> link_names_short(link_names.begin() + 1,
                                            link_names.end() - 1);
  expected[1] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.707107, 0.000000, 0.707107),
            Vector(0.000000, 0.220941, 0.127300));
  expected[2] =
      Frame(KDL::Rotation::Quaternion(0.000000, 0.707107, 0.000000, 0.707107),
            Vector(0.612000, 0.049041, 0.127300));
  expected[3] =
      Frame(KDL::Rotation::Quaternion(0.000000, 1.000000, 0.000000, 0.000000),
            Vector(1.184300, 0.049041, 0.127300));
  expected[4] =
      Frame(KDL::Rotation::Quaternion(0.000000, 1.000000, 0.000000, 0.000000),
            Vector(1.184300, 0.163941, 0.127300));
  ...
  EXPECT_TRUE(kin.linkTransforms(joint_angles, actual,
                                 link_names_short)); // all link names
...
TEST_F(RobotTest, calcFwdKinInputValidation) {
  ...
  EXPECT_FALSE(
      BasicKin().calcFwdKin(VectorXd::Zero(6), pose));    // un-init BasicKin
  EXPECT_FALSE(kin.calcFwdKin(VectorXd(), pose));         // empty joints
  EXPECT_FALSE(kin.calcFwdKin(VectorXd::Zero(99), pose)); // too many joints
  EXPECT_FALSE(
      kin.calcFwdKin(VectorXd::Constant(6, 1e10), pose)); // joints out-of-range
...
TEST_F(RobotTest, calcJacobianInputValidation) {
  ...
  EXPECT_FALSE(
      BasicKin().calcJacobian(VectorXd(), jacobian)); // un-init BasicKin & Jnts
  EXPECT_FALSE(
      BasicKin().calcJacobian(VectorXd::Zero(6), jacobian)); // un-init BasicKin
  EXPECT_FALSE(kin.calcJacobian(VectorXd(), jacobian));      // empty joints
  EXPECT_FALSE(
      kin.calcJacobian(VectorXd::Zero(99), jacobian)); // too many joints
  EXPECT_FALSE(kin.calcJacobian(VectorXd::Constant(6, 1e10),
                                jacobian)); // joints out-of-range
...
TEST_F(RobotTest, calcJacobianKnownPoses) {
  ...
  // use random joint states, compute the numerical Jacobian and compare to that
  // computed
  ...
  for (int j = 0; j < 10; j++) // try 10 different poses
  {
    ...
    for (int i = 0; i < (int)joints.size(); i++) {
      ...
      double delta_y =
          (updated_pose.translation().y() - pose.translation().y()) / delta;
      double delta_z =
          (updated_pose.translation().z() - pose.translation().z()) / delta;
      EXPECT_NEAR(delta_x, jacobian(0, i), 1e-3);
      EXPECT_NEAR(delta_y, jacobian(1, i), 1e-3);
      EXPECT_NEAR(delta_z, jacobian(2, i), 1e-3);
      Eigen::AngleAxisd r12(pose.rotation().transpose() *
                            updated_pose.rotation()); // rotation from p1 -> p2
      ...
      if (theta > M_PI)
        theta = theta - 2. * M_PI;
      Eigen::VectorXd omega = (pose.rotation() * r12.axis() * theta) /
                              delta; // axis k * theta expressed in frame0
...
TEST_F(PInvTest, solvePInvInputValidation) {
  ...
  EXPECT_FALSE(kin.solvePInv(MatrixXd(), VectorXd(),
                             vResult)); // will generate ROS_ERROR("Empty
                                        // matrices not supported in
                                        // solvePinv()");
  EXPECT_FALSE(kin.solvePInv(
      MatrixXd(6, 3), VectorXd(1),
      vResult)); // will generate ROS_ERROR("Matrix size mismatch:...");
...
/** @brief This test exercises the BasicKin solvePInv function with random
 * inputs */
