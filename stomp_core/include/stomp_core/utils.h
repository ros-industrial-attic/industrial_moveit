 ...
 */
struct Rollout {
  Eigen::MatrixXd noise; /**< @brief A matrix [num_dimensions][num_time_steps]
                            of random noise applied to the parameters*/
  Eigen::MatrixXd parameters_noise; /**< @brief A matrix
                                       [num_dimensions][num_time_steps] of the
                                       sum of parameters + noise */

  Eigen::VectorXd
      state_costs; /**< @brief A vector [num_time_steps] of the cost at each
                      timestep */
  Eigen::MatrixXd control_costs; /**< @brief A matrix
                                    [num_dimensions][num_time_steps] of the
  ...
  Eigen::
      MatrixXd total_costs;      /**< @brief A matrix
                                    ...
                                    control_costs_[d]*/
  Eigen::MatrixXd probabilities; /**< @brief A matrix
                                    [num_dimensions][num_time_steps] of the
                                    probability for each parameter at every
                                    timestep */

  std::vector<double>
      full_probabilities;         /**< @brief A vector [num_dimensions] of the
                                     probabilities for the full trajectory */
  std::vector<double> full_costs; /**< @brief A vector [num_dimensions] of the
                                     full coss, state_cost + control_cost for
                                     each joint over the entire trajectory
                                       full_costs_[d] = state_cost.sum() +
                                     control_cost[d].sum() */

  double importance_weight; /**< @brief importance sampling weight */
  double total_cost; /**< @brief combined state + control cost over the entire
                        trajectory for all joints */
};
...
enum DerivativeOrder {
  ...
  STOMP_ACCELERATION =
      2,         /**< Calculate acceleration using finite differentiation */
...
enum TrajectoryInitialization {
  LINEAR_INTERPOLATION =
      1, /**< Calculate initial trajectory using linear interpolation */
  CUBIC_POLYNOMIAL_INTERPOLATION, /**< Calculate initial trajectory using cubic
                                     polynomial interpolation */
  MININUM_CONTROL_COST, /**< Calculate initial trajectory using minimum control
                           cost */
  FILL_TRAJECTORY /**< Fill the initial trajectory using that obtained from a
                     different motion planner */
};
}
...
struct StompConfiguration {
  ...
  int num_iterations_after_valid; /**< @brief Stomp will stop optimizing this
                                     many iterations after finding a valid
                                     solution */
  ...
  int initialization_method; /**< @brief
                                TrajectoryInitializations::TrajectoryInitialization
                                */

  // Probability Calculation
  double exponentiated_cost_sensitivity; /**< @brief Default exponetiated cost
                                            sensitivity coefficient */

  // Noisy trajectory generation
  int num_rollouts; /**< @brief Number of noisy trajectories*/
  int max_rollouts; /**< @brief The combined number of new and old rollouts
                       during each iteration shouldn't exceed this value */

  // Cost calculation
  double control_cost_weight; /**< @brief Percentage of the trajectory
                                 accelerations cost to be applied in the total
                                 cost calculation >*/
};
...
/** @brief Contains the coefficients for each of the finite central
 * differentiation (position, velocity, acceleration, and jerk) */
static const double
    FINITE_CENTRAL_DIFF_COEFFS[FINITE_DIFF_RULE_LENGTH]
                              [FINITE_DIFF_RULE_LENGTH] = {
                                  {0, 0, 0, 1, 0, 0, 0}, // position
                                  {0, 1.0 / 12.0, -2.0 / 3.0, 0, 2.0 / 3.0,
                                   -1.0 / 12.0, 0}, // velocity
                                  {0, -1 / 12.0, 16 / 12.0, -30 / 12.0,
                                   16 / 12.0, -1 / 12.0,
                                   0}, // acceleration (five point stencil)
                                  {0, 1 / 12.0, -17 / 12.0, 46 / 12.0,
                                   -46 / 12.0, 17 / 12.0, -1 / 12.0} // jerk
};

/** @brief Contains the coefficients for each of the finite forward
 * differentiation (position, velocity, acceleration, and jerk) */
static const double
    FINITE_FORWARD_DIFF_COEFFS[FINITE_DIFF_RULE_LENGTH]
                              [FINITE_DIFF_RULE_LENGTH] = {
                                  {1, 0, 0, 0, 0, 0, 0}, // position
                                  {-25.0 / 12.0, 4.0, -3.0, 4.0 / 3.0,
                                   -1.0 / 4.0, 0, 0}, // velocity
                                  {15.0 / 4.0, -77.0 / 6.0, 107.0 / 6.0, -13.0,
                                   61.0 / 12.0, -5.0 / 6.0,
                                   0}, // acceleration (five point stencil)
                                  {-49 / 8, 29, -461 / 8, 62, -307 / 8, 13,
                                   -15 / 8} // jerk
};
...
void differentiate(const Eigen::VectorXd &parameters,
                   DerivativeOrders::DerivativeOrder order, double dt,
 ...
 */
void generateSmoothingMatrix(int num_time_steps, double dt,
                             Eigen::MatrixXd &projection_matrix_M);
