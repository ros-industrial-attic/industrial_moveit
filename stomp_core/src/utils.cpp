...
void generateFiniteDifferenceMatrix(int num_time_steps,
  ...
  for (int i = 0; i < num_time_steps; ++i) {
    for (int j = -FINITE_DIFF_RULE_LENGTH / 2; j <= FINITE_DIFF_RULE_LENGTH / 2;
         ++j) {
      int index = i + j;
      if (index < 0) {
        index = 0;
      ...
      diff_matrix(i, index) =
          multiplier *
          FINITE_CENTRAL_DIFF_COEFFS[order][j + FINITE_DIFF_RULE_LENGTH / 2];
    }
  }
}

void generateSmoothingMatrix(int num_timesteps, double dt,
  ...
  generateFiniteDifferenceMatrix(num_timesteps_padded,
                                 DerivativeOrders::STOMP_ACCELERATION, dt,
                                 finite_diff_matrix_A_padded);
   ...
   */
  MatrixXd control_cost_matrix_R_padded =
      dt * finite_diff_matrix_A_padded.transpose() *
      finite_diff_matrix_A_padded;
  ...
  MatrixXd inv_control_cost_matrix_R =
      control_cost_matrix_R.fullPivLu().inverse();
  ...
  for (auto t = 0u; t < num_timesteps; t++) {
    ...
    projection_matrix_M.col(t) *=
        (1.0 / (num_timesteps *
                max)); // scaling such that the maximum value is 1/num_timesteps
  }
}
...
void differentiate(const Eigen::VectorXd &parameters,
  ...
  // coefficient arrays
  VectorXd central_coeffs = VectorXd::Map(&FINITE_CENTRAL_DIFF_COEFFS[order][0],
                                          FINITE_DIFF_RULE_LENGTH);
  VectorXd forward_coeffs = VectorXd::Map(&FINITE_FORWARD_DIFF_COEFFS[order][0],
                                          FINITE_DIFF_RULE_LENGTH);
