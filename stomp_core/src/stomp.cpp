...
/**
 * @brief initializes the initial stomp trajectory to be the trajectory obtained
 * from another motion planner like CHOMP, OMPL, etc.
 * @param trajectory_points trajectory to be initialized which would be used in
 * STOMP
 * @param trajectory_points2 trajectory obtained from another motion planner to
 * be populated into trajectory_points
 */
static void fillFromTrajectory(Eigen::MatrixXd &trajectory_points,
                               Eigen::MatrixXd trajectory_points2) {

  // num_rows_trajectory1 represents the number of points in the trajectory to
  // be used in STOMP planner's process
  int num_rows_trajectory1 = trajectory_points.cols();
  // num_rows_trajectory2 represents the number of points in the trajectory
  // generated by another planner like OMPL, CHOMP, etc.
  int num_rows_trajectory2 = trajectory_points2.rows();

  // num_joints is the number of joints in the given robot's trajectory
  int num_joints = trajectory_points.rows();
  ...
  if (num_rows_trajectory1 > num_rows_trajectory2) {
    ...
    for (int i = 0; i < num_rows_trajectory2; i++) {
      for (int k = 0; k < repeated_factor; k++) {
        assignJointsInTrajectory(trajectory_points, trajectory_points2,
                                 response_point_id, i);
        response_point_id++;
      }
    ...
    for (int i = 0; i < num_rows_trajectory1; i++) {
      ...
      assignJointsInTrajectory(trajectory_points, trajectory_points2, i,
                               sampled_point);
    }
  } else {
    for (int i = 0; i < num_rows_trajectory1; i++) {
      assignJointsInTrajectory(trajectory_points, trajectory_points2, i, i);
    }
  }
}
...
bool Stomp::computeInitialTrajectory(const std::vector<double> &first,
  ...
  case TrajectoryInitializations::FILL_TRAJECTORY:
    ...
    fillFromTrajectory(parameters_optimized_, original_trajectory);
    break;
  }
