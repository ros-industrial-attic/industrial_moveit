...
static const double DEFAULT_NOISY_COST_IMPORTANCE_WEIGHT =
    1.0; /**< Default noisy cost importance weight */
static const double MIN_COST_DIFFERENCE =
    1e-8; /**< Minimum cost difference allowed during probability calculation */
static const double MIN_CONTROL_COST_WEIGHT =
    1e-8; /**< Minimum control cost weight allowed */
 ...
 * @param last              The final position
 * @param num_timesteps     The number of timesteps
 * @param initial_trajectory The original trajectory to be copied
 * @param trajectory_joints The returned linear interpolated trajectory
 */

static void fillFromTrajectory(const std::vector<double> &first,
                               const std::vector<double> &last,
                               int num_timesteps,
                               Eigen::MatrixXd &trajectory_joints,
...
static void computeCubicInterpolation(const std::vector<double> &first,
  ...
  for (int unsigned i = 0; i < first.size(); i++) {
    ...
    for (unsigned j = 0; j < num_points; j++) {
      t = j * dt;
      trajectory_joints(i, j) =
          coeffs[0] + coeffs[2] * pow(t, 2) + coeffs[3] * pow(t, 3);
    }
  }
}
...
bool computeMinCostTrajectory(
    const std::vector<double> &first, const std::vector<double> &last,
  ...
  if (control_cost_matrix_R_padded.rows() !=
      control_cost_matrix_R_padded.cols()) {
    ROS_ERROR("Control Cost Matrix is not square");
    return false;
  }

  int timesteps =
      control_cost_matrix_R_padded.rows() - 2 * (FINITE_DIFF_RULE_LENGTH - 1);
  int start_index_padded = FINITE_DIFF_RULE_LENGTH - 1;
  int end_index_padded = start_index_padded + timesteps - 1;
  std::vector<Eigen::VectorXd> linear_control_cost(
      first.size(), Eigen::VectorXd::Zero(timesteps));
  trajectory_joints.setZero(first.size(), timesteps);

  for (unsigned int d = 0; d < first.size(); d++) {
    linear_control_cost[d].transpose() =
        first[d] *
        Eigen::VectorXd::Ones(FINITE_DIFF_RULE_LENGTH - 1).transpose() *
        control_cost_matrix_R_padded.block(
            0, start_index_padded, FINITE_DIFF_RULE_LENGTH - 1, timesteps);
    ...
    trajectory_joints.row(d) =
        -0.5 * inv_control_cost_matrix_R * linear_control_cost[d];
 ...
 */
void computeParametersControlCosts(const Eigen::MatrixXd &parameters, double dt,
  ...
  for (auto d = 0u; d < parameters.rows(); d++) {
    cost = double(parameters.row(d) *
                  (control_cost_matrix_R * parameters.row(d).transpose()));
...
bool Stomp::solve(const std::vector<double> &first,
  ...
  // a new argument needs to be added for the process of loading an existing
  // trajectory
  // which is obtained from another planner like CHOMP, OMPL, etc.
  // the argument to be added is EigenXd existing_trajectory
  // Eigen::MatrixXd temp_original_trajectory;
  if (!computeInitialTrajectory(first, last, temp_original_trajectory)) {
...
bool Stomp::solve(const Eigen::VectorXd &first, const Eigen::VectorXd &last,
                  Eigen::MatrixXd &parameters_optimized,
                  Eigen::MatrixXd original_trajectory) {
  ...
  return solve(start, end, parameters_optimized, original_trajectory);
}
...
bool Stomp::solve(const Eigen::MatrixXd &initial_parameters,
  ...
  if (initial_parameters.rows() != config_.num_dimensions ||
      initial_parameters.cols() != config_.num_timesteps) {
  ...
  while (current_iteration_ <= config_.num_iterations && runSingleIteration()) {
    ...
    ROS_DEBUG("STOMP completed iteration %i with cost %f", current_iteration_,
              current_lowest_cost_);
  ...
  if (parameters_valid_) {
    ...
    if (proceed_)
      ROS_ERROR("STOMP failed to find a valid solution after %i iterations",
                current_iteration_);
    else
...
bool Stomp::resetVariables() {
  ...
  if (config_.max_rollouts <= config_.num_rollouts) {
    ...
    config_.max_rollouts = config_.num_rollouts +
                           1; // one more to accommodate optimized trajectory
  }
  ...
  num_timesteps_padded_ =
      config_.num_timesteps + 2 * (FINITE_DIFF_RULE_LENGTH - 1);
  generateFiniteDifferenceMatrix(num_timesteps_padded_,
                                 DerivativeOrders::STOMP_ACCELERATION,
                                 config_.delta_t, finite_diff_matrix_A_padded_);

  /* control cost matrix (R = A_transpose * A):
   * Note: Original code multiplies the A product by the time interval.  However
   * this is not
   * what was described in the literature
   */
  control_cost_matrix_R_padded_ = config_.delta_t *
                                  finite_diff_matrix_A_padded_.transpose() *
                                  finite_diff_matrix_A_padded_;
  control_cost_matrix_R_ = control_cost_matrix_R_padded_.block(
      start_index_padded_, start_index_padded_, config_.num_timesteps,
  ...
  inv_control_cost_matrix_R_ /=
      maxVal; // used in computing the minimum control cost initial trajectory

  return true;
}

// It should be noted here that the computeInitialTrajectory needs to have an
// additional argument which stores the initial trajectory to be loaded as a
// seed trajectory

bool Stomp::computeInitialTrajectory(const std::vector<double> &first,
                                     const std::vector<double> &last,
                                     Eigen::MatrixXd original_trajectory) {
  bool valid = true;
  ...
  case TrajectoryInitializations::CUBIC_POLYNOMIAL_INTERPOLATION:
    ...
    computeCubicInterpolation(first, last, config_.num_timesteps,
                              config_.delta_t, parameters_optimized_);
    break;
  case TrajectoryInitializations::LINEAR_INTERPOLATION:

    computeLinearInterpolation(first, last, config_.num_timesteps,
                               parameters_optimized_);
    break;
  case TrajectoryInitializations::MININUM_CONTROL_COST:

    valid = computeMinCostTrajectory(first, last, control_cost_matrix_R_padded_,
                                     inv_control_cost_matrix_R_,
                                     parameters_optimized_);
    break;
...
bool Stomp::runSingleIteration() {
  ...
  bool proceed = generateNoisyRollouts() && computeNoisyRolloutsCosts() &&
                 filterNoisyRollouts() && computeProbabilities() &&
                 updateParameters() && computeOptimizedCost();

  // notifying end of iteration
  task_->postIteration(0, config_.num_timesteps, current_iteration_,
                       current_lowest_cost_, parameters_optimized_);
...
bool Stomp::generateNoisyRollouts() {
  ...
  std::vector<std::pair<double, int>> rollout_cost_sorter; // Used to sort noisy
                                                           // trajectories in
                                                           // ascending order
                                                           // wrt their total
                                                           // cost
  ...
  int rollouts_stored =
      num_active_rollouts_ - 1; // don't take the optimized rollout into account
  ...
  int rollouts_reuse =
      rollouts_total < config_.max_rollouts
          ? rollouts_stored
          : config_.max_rollouts -
                (rollouts_generate + 1); // +1 for optimized params
  ...
  if (rollouts_reuse > 0) {
    ...
    for (auto r = 0u; r < rollouts_stored; ++r) {
      ...
      // Apply noise generated on the previous iteration onto the current
      // trajectory
      noisy_rollouts_[r].noise =
          noisy_rollouts_[r].parameters_noise - parameters_optimized_;

      cost_prob =
          exp(-h * (noisy_rollouts_[r].total_cost - min_cost) / cost_denom);
  ...
  noisy_rollouts_[rollouts_generate + rollouts_reuse].parameters_noise =
      parameters_optimized_;
  noisy_rollouts_[rollouts_generate + rollouts_reuse].noise.setZero();
  noisy_rollouts_[rollouts_generate + rollouts_reuse].state_costs =
      parameters_state_costs_;
  noisy_rollouts_[rollouts_generate + rollouts_reuse].control_costs =
      parameters_control_costs_;

  // generate new noisy rollouts
  for (auto r = 0u; r < rollouts_generate; r++) {
    if (!task_->generateNoisyParameters(
            parameters_optimized_, 0, config_.num_timesteps, current_iteration_,
            r, noisy_rollouts_[r].parameters_noise, noisy_rollouts_[r].noise)) {
      ROS_ERROR("Failed to generate noisy parameters at iteration %i",
                current_iteration_);
      return false;
    }
  }
...
bool Stomp::filterNoisyRollouts() {
  ...
  for (auto r = 0u; r < config_.num_rollouts; r++) {
    if (!task_->filterNoisyParameters(
            0, config_.num_timesteps, current_iteration_, r,
            noisy_rollouts_[r].parameters_noise, filtered)) {
...
bool Stomp::computeRolloutsStateCosts() {
  ...
  for (auto r = 0u; r < config_.num_rollouts; r++) {
    ...
    if (!task_->computeNoisyCosts(rollout.parameters_noise, 0,
                                  config_.num_timesteps, current_iteration_, r,
...
bool Stomp::computeRolloutsControlCosts() {
  ...
  for (auto r = 0u; r < num_active_rollouts_; r++) {
    ...
    if (config_.control_cost_weight < MIN_CONTROL_COST_WEIGHT) {
      ...
      computeParametersControlCosts(rollout.parameters_noise, config_.delta_t,
...
bool Stomp::computeProbabilities() {
  ...
  double numerator;
  double probl_sum =
      0.0; // total probability sum of all rollouts for each joint
  ...
  for (auto d = 0u; d < config_.num_dimensions; ++d) {
    ...
    for (auto t = 0u; t < config_.num_timesteps; t++) {
      ...
      probl_sum = 0.0;
      for (auto r = 0u; r < num_active_rollouts_; ++r) {
        // this is the exponential term in the probability calculation described
        // in the literature
        exponent =
            -h * (noisy_rollouts_[r].total_costs(d, t) - min_cost) / denom;
        noisy_rollouts_[r].probabilities(d, t) =
            noisy_rollouts_[r].importance_weight * exp(exponent);

        probl_sum += noisy_rollouts_[r].probabilities(d, t);
      }

      // scaling each probability value by the sum of all probabilities
      // corresponding to all rollouts at time "t"
      for (auto r = 0u; r < num_active_rollouts_; ++r) {
    ...
    for (int r = 0; r < num_active_rollouts_; ++r) {
      noisy_rollouts_[r].full_probabilities[d] =
          noisy_rollouts_[r].importance_weight *
...
bool Stomp::updateParameters() {
  ...
  for (auto d = 0u; d < config_.num_dimensions; d++) {
    ...
    for (auto r = 0u; r < num_active_rollouts_; r++) {
      ...
      parameters_updates_.row(d) +=
          (rollout.noise.row(d).array() * rollout.probabilities.row(d).array())
              .matrix();
    }
  }

  // filtering updates
  if (!task_->filterParameterUpdates(0, config_.num_timesteps,
                                     current_iteration_, parameters_optimized_,
                                     parameters_updates_)) {
...
bool Stomp::computeOptimizedCost() {
  ...
  parameters_total_cost_ = 0;
  if (config_.control_cost_weight > MIN_CONTROL_COST_WEIGHT) {
    computeParametersControlCosts(
        parameters_optimized_, config_.delta_t, config_.control_cost_weight,
        control_cost_matrix_R_, parameters_control_costs_);
  ...
  // state costs
  if (task_->computeCosts(parameters_optimized_, 0, config_.num_timesteps,
                          current_iteration_, parameters_state_costs_,
                          parameters_valid_)) {
