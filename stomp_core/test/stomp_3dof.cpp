...
const std::vector<double> END_POS = {-1.25, 1.0,
                                     -0.26}; /**< Trajectory ending posiiton */
...
public:
  /**
   * @brief A dummy task for testing Stomp
   * @param parameters_bias default parameter bias used for computing cost for
   * the test
   * @param bias_thresholds threshold to determine whether two trajectories are
   * equal
  ...
  DummyTask(const Trajectory &parameters_bias,
            ...
            const std::vector<double> &std_dev)
      : parameters_bias_(parameters_bias), bias_thresholds_(bias_thresholds),
        std_dev_(std_dev) {
  ...
  bool generateNoisyParameters(const Eigen::MatrixXd &parameters,
                               ...
                               std::size_t num_timesteps, int iteration_number,
                               int rollout_number,
    ...
    for (std::size_t d = 0; d < parameters.rows(); d++) {
      for (std::size_t t = 0; t < parameters.cols(); t++) {
        rand_noise = static_cast<double>(rand() % RAND_MAX) /
                     static_cast<double>(RAND_MAX - 1); // 0 to 1
  ...
  bool computeCosts(const Trajectory &parameters, std::size_t start_timestep,
                    std::size_t num_timesteps, int iteration_number,
  ...
  bool computeNoisyCosts(const Trajectory &parameters,
                         std::size_t start_timestep, std::size_t num_timesteps,
                         int iteration_number, int rollout_number,
                         Eigen::VectorXd &costs, bool &validity) override {
  ...
  bool filterParameterUpdates(std::size_t start_timestep,
    ...
    return smoothParameterUpdates(start_timestep, num_timesteps,
                                  iteration_number, updates);
  }
...
protected:
  Trajectory parameters_bias_; /**< Parameter bias used for computing cost for
                                  the test */
  std::vector<double> bias_thresholds_; /**< Threshold to determine whether two
                                           trajectories are equal */
  std::vector<double>
      std_dev_; /**< Standard deviation used for generating noisy parameters */
...
/** @brief This tests the Stomp solve method using a linear interpolated inital
 * trajectory */
...
/** @brief This tests the Stomp solve method using a cubic polynomial
 * interpolated inital trajectory */
...
/** @brief This tests the Stomp solve method using a minimum cost inital
 * trajectory */
...
TEST(Stomp3DOF, solve_60_timesteps) {
  ...
  StompConfiguration config = create3DOFConfiguration();
  // config.initialization_method =
  // TrajectoryInitializations::MININUM_CONTROL_COST; // this initialization
  // method is currently behaving oddly
  config.num_iterations = 100;
  config.num_timesteps = num_timesteps;
  Stomp stomp(config, task);
